...
def run_opencode(model: str, prompt: str):
...
def score_essay(model: str, question: str, rubric: str, essay: str, max_rating: int):
    prompt = build_prompt(question, rubric, essay)
    try:
        proc = run_opencode(model, prompt)
...
def run_set(ds_name, ds, set_num, model, test_only):
    ...
    for _, r in tqdm(df.iterrows(), total=len(df), desc=f"{ds_name}/Set{set_num}"):
        ...
        pred, raw = score_essay(model, question, rubric, essay, max_rating)
        rows.append(
            {
                "EssayText": essay,
                "PredScore": pred,
...
if __name__ == "__main__":
    ...
    for ds_name, ds in DATASETS.items():
        ...
        if not sets:
            ...
            continue
        for set_num in sets:
            run_set(ds_name, ds, set_num, args.model, test_only)
